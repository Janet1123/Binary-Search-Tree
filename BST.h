...
struct BST {
		...
		//self created function in this assignment
		int Depth(BTNode*);
		void printDeepestNode(BTNode*, int, int);
		bool deepestNodes();

		void Ascending(BTNode*, ostream&);
		void Descending(BTNode*, ostream&);
		bool display(int, int);

		bool CloneSubtree(BST, type);
		void CloneSubtree2(BTNode*, type);
		void CloneSubtree3(BTNode*, BTNode*);

		bool printLevelNodes();

		bool printPath();
		bool printPathRecur(BTNode*, int[], int);
		void printPath2(int[], int);
		void TreeOperation(BTNode*);
};
